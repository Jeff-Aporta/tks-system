...
`,I=(i,e,t="")=>({nombre:i,path:e,hijos:new Map,pista:t||void 0}),it=(i,e)=>{let t=I("","");for(let r of i){let o=String(r).split(/[/\\]/).filter(Boolean),a=t,s=[];o.forEach((l,c)=>{s.push(l);let h=s.join("/");if(a.hijos.has(l)){if(c===o.length-1){let g=a.hijos.get(l),f=String(g.pista??e[h]??e[r]??e[l]??"");f&&!g.pista&&a.hijos.set(l,I(l,h,f))}}else{let f=c===o.length-1?String(e[h]??e[r]??e[l]??e[o.slice(0,c+1).join("/")]??""):"";a.hijos.set(l,I(l,h,f))}a=a.hijos.get(l)})}return t},de=(i,e,t)=>{let r=d(i),o=String(r.name??r.nombre??"").trim();if(!o)return null;let a=String(r.path??(t?`${t}/${o}`:o)),s=Array.isArray(r.children)?r.children:Array.isArray(r.hijos)?r.hijos:[],l=I(o,a,String(r.hint??r.pista??e[a]??e[o]??""));for(let c of s){let h=de(c,e,a);h&&l.hijos.set(h.nombre,h)}return l},rt=(i,e)=>{let t=I("","");for(let r of i){let o=de(r,e,"");o&&t.hijos.set(o.nombre,o)}return t},me=0,ue=i=>{let e=i.hijos.size===0,t=e?"mdi:file-document-outline":"mdi:folder-outline",r=i.pista?`ft-tip-${++me}`:"";return n`
...
`},W={cdn:ve,async html(i){return It(i)},async descargar(i){let e=await W.html(i),t=new Blob([e],{type:"text/html;charset=utf-8"}),r=URL.createObjectURL(t),o=document.createElement("a");o.href=r,o.download=`${i.iticket}.html`,document.body.append(o),o.click(),o.remove(),setTimeout(()=>URL.revokeObjectURL(r),3e4)}};var Mt=`
  :host {
    display: none;
    flex-wrap: nowrap;
    gap: 0.15rem;
...
`,G=class extends HTMLElement{#t=null;#e;constructor(){super(),this.#e=this.attachShadow({mode:"open"}),k(this.#e,Mt)}connectedCallback(){this.#r()}get ticket(){return this.#t}set ticket(e){this.#t=e,this.toggleAttribute("activo",!!e?.iticket),this.isConnected&&this.#r()}async#i(){let e=this.#t;if(!e)return;let t=fe.enlace({space:e.space,tk:e.iticket,full:!0}),r=navigator;if(r.share)try{await r.share({title:`${e.iticket} \xB7 ${e.titulo??""}`.trim(),url:t});return}catch{}try{await navigator.clipboard.writeText(t),C("Enlace copiado al portapapeles.","success")}catch{C("No se pudo copiar el enlace. C\xF3pialo de la barra de direcciones.","warning")}}async#o(e){let t=this.#t;if(t){e.setAttribute("loading","");try{await W.descargar(t),C(`${t.iticket}.html descargado.`,"success")}catch(r){C(`No se pudo generar el HTML: ${r instanceof Error?r.message:r}`,"danger")}finally{e.removeAttribute("loading")}}}#r(){for(;this.#e.firstChild;)this.#e.removeChild(this.#e.firstChild);this.#t?.iticket&&this.#e.append(n`
      <is-button
        variant="text"
        color="neutral"
        pill
